 ...
 */
export function NotifyBanner({ title, body, when }: { title: string; body: string; when: string }) {
  return (
    <View
      style={{
    ...
    >
      <View style={{ flexDirection: "row", alignItems: "center", gap: tokens.space.sm }}>
        ...
        />
        <Text style={{ ...tokens.text.legend, color: tokens.color.textMuted, flex: 1 }}>
          Wrenchy
        </Text>
        <Text style={{ ...tokens.text.legend, color: tokens.color.textFaint }}>{when}</Text>
      </View>
